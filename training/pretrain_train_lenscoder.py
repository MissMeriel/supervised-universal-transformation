...
def parse_arguments():
    ...
    parser.add_argument("-p", '--pretrained_weights', help='pretrained VAE weights for warm-start')
    ...
    print("ARGS:", args.pretrained_weights, args.training_dataset, args.validation_dataset, args.procid, args.epochs, args.loss_fn, args.lr)
    return args
...
NAME = f"all-fulltrain-Lenscoder-{args.loss_fn}loss-{args.procid}-{timestr}-{randstr}"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, hashmap in enumerate(data_loader, start=1):
            ...
            if args.loss_fn == "orig":
                ...
                loss, rloss, kloss = loss_fn_prediction_kld(recons, y, mu, log_var, kld_weight)
            loss.backward()
...
def main():
    ...
    model = torch.load(args.pretrained_weights)
