...
def parse_arguments():
    ...
    print("ARGS:", args.training_dataset, args.validation_dataset, args.procid, args.epochs, args.loss_fn, args.lr, args.latent_dim)
    return args
...
NAME = f"{dataset_id}-Lenscoder-empiricalTEST_{args.loss_fn}loss-{args.procid}-{timestr}-{randstr}"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, hashmap in enumerate(data_loader, start=1):
            ...
            if (i % sample_interval) == 0:
                ...
                print(f"\t{tot_recons_loss=}")
                print(f"\t{(tot_pred_loss > 0)=}")
                print(f"\t{tot_kld_loss.shape=}")
