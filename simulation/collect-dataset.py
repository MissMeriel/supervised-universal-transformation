...
lanewidth = 3.75 #2.25
centerline = []
centerline_interpolated, roadleft, roadright = [], [], []
cutoff = None
...
def road_analysis(bng, road_id):
    global centerline, roadleft, roadright, cutoff
...
def collect_dataset(vehicle, bng, default_scenario, parent_dir, road_id, reverse=False, seg=None):
    global base_filename
    global integral, prev_error, setpoint
    integral = 0.0
    prev_error = 0.0
    ...
    wheelspeed = runtime = kph = throttle = damage = distance_from_center = 0.0
    ...
    writedir = os.path.join(parent_dir, f"{default_scenario}-{road_id}-lap-test")
    ...
    with open(f"{writedir}/data.txt", "w") as f:
        ...
        while damage <= 1:
            ...
            if distance2D(vehicle.state["pos"], cutoff) < 12:
...
def get_transf(transf_id):
    if transf_id is None:
        ...
        img_dims = (240,135); fov=61; transf = "fisheye" # previously 75
...
def main():
    topo_id = "Rturn"
    transf_id = "fisheye"
    parent_dir = "F:/supervised-universal-transformation-datasets/"
    default_scenario, road_id, seg = get_topo(topo_id)
    img_dims, fov, transf = get_transf(transf_id)

    vehicle, bng, scenario = setup_beamng(default_scenario=default_scenario, road_id=road_id, seg=seg, img_dims=img_dims, fov=fov, vehicle_model='etk500',
                                          beamnginstance='C:/Users/Meriel/Documents/BeamNG.researchINSTANCE3', port=64556)
    distances, deviations, trajectories = [], [], []
    results = collect_dataset(vehicle, bng, scenario, parent_dir, default_scenario=default_scenario, road_id=road_id, seg=seg)
    bng.close()
