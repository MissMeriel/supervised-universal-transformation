...
def main(args):
    ...
    curr_topo = None
    for topoDir in allDirs:
        pcks = [f"{topoDir}/{_}" for _ in os.listdir(topoDir) if _.endswith(fileExt)]
        topo_devs = []
        topo_dists = []
        topo_cluster_id = []
        topo_track_dists = []
        topo = topoDir.split("/")[-1]
        for pck in pcks:
            results = unpickle_results(pck)
            topo_devs.append(results["deviation"])
            topo_dists.append(results["traj_len"])
            topo_track_dists.append(results["ctrline_length"])
            topo_cluster_id.append(pck)
        idxs = np.array(topo_devs).argsort()
        print(topo)
        for i in range(1, 6):
            print(topo_cluster_id[idxs[len(idxs)-i]].split("/")[-1])
            print(f"Track seg.length=\t{topo_track_dists[idxs[len(idxs)-i]]:.1f}")
            print(f"Trajectory dist=\t{topo_dists[idxs[len(idxs)-i]]:.1f}")
            print(f"Center deviation=\t{topo_devs[idxs[len(idxs)-i]]:.3f}")
